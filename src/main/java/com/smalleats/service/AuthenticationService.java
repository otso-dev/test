...
public class AuthenticationService implements UserDetailsService {
    ...
    public Map<String,String> login(LoginReqDto loginReqDto, HttpServletResponse response){
        ...
        if(loginReqDto.getRole().equals("USER") || loginReqDto.getRole().equals("ADMIN")){
        ...
        PartnerUser partnerUser = userDAO.findPartnerUserByEmail(loginReqDto.getEmail());
        if(partnerUser == null){
            throw new CustomException("로그인 실패",ErrorMap.builder().put("login","사용자 정보를 확인하세요").build());
        }
        BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
        if(!passwordEncoder.matches(loginReqDto.getPassword(),partnerUser.getPartnerPassword())){
            throw new CustomException("로그인 실패", ErrorMap.builder().put("login","사용자 정보를 확인하세요").build());
        }
        UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(
                loginReqDto.getEmail(),loginReqDto.getPassword()
